from pathlib import Path
from typing import Dict, List, Optional, Sequence
from src.converters.base_converter import BaseConverter
from src.core.types import Artifact, ArtifactType, ProcessOptions, ProcessResult
from src.core.errors import ConverterError
from src.providers.pdf.base import IPdfProvider


class PDFConverter(BaseConverter):
    name = "pdf"
    suffixes = {".pdf"}

    def __init__(
        self,
        providers: Sequence[IPdfProvider],
        prefer: Optional[str] = None,
    ):
        super().__init__()
        self._providers = providers
        assert providers, "至少需要一個 PDF 解析提供者"
        self.logger.info(f"Initialized PDFConverter with providers: {[p.name for p in providers]}")
        self._prefer = prefer # provider.name

    def convert_files(
            self,
            input_paths: Sequence[Path],
            *,
            output_root: Path,
            options: Optional[ProcessOptions] = None
        ) -> Dict[Path, ProcessResult]:
        options = options or ProcessOptions()
        if isinstance(input_paths, list):
            input_paths = [Path(p) for p in input_paths]

        pdfs = [p for p in input_paths if self.supports(p)]
        if not pdfs:
            self.logger.warning("No supported PDF files found in input.")
            return {}

        provider_name = (options.extra.get("pdf_provider") if options else None) or self._prefer
        candidates = self._select_providers(provider_name)
        self.logger.info(
                    "PDF converting %d files to %s using providers: %s",
                    len(pdfs), output_root, [p.name for p in candidates]
                )
        
        last_err: Optional[Exception] = None
        for provider in candidates:
            try:
                res = provider.convert_pdfs(pdfs, output_root=output_root, options=options)
                missing = [p for p in pdfs if p not in res]
                if missing:
                    self.logger.warning(f"Provider {provider.name} did not return results for: {missing}")
                else:
                    self.logger.info(f"Provider {provider.name} successfully processed all PDFs.")

                return_dict = options.extra.get("return_dict", True)
                if return_dict:
                    res = self.result_map_to_plain_dict(res)
                return res
            except Exception as e:
                self.logger.error(f"Provider {provider.name} failed with error: {e}")
                last_err = e
                continue
        raise ConverterError(f"All PDF providers failed. Last error: {last_err}")

    def _select_providers(self, preferred: Optional[str]) -> List[IPdfProvider]:
        if preferred:
            ordered = [p for p in self._providers if p.name == preferred] + \
                      [p for p in self._providers if p.name != preferred]
        else:
            ordered = list(self._providers)
        return ordered
